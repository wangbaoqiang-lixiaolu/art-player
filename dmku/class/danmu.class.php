...
class danmu {
    ...
    public function 弹幕池2($id){
        ...
        foreach($data as $k => $v){
            // 请不要随意调换下列数组赋值顺序
            $arr[$k]['text'] = (string)$v['text'];  //弹幕文本
            $arr[$k]['time'] = (float)$v['videotime'];  //弹幕出现时间(s)
            $arr[$k]['color'] = (string)$v['color']; //字体的颜色
            $arr[$k]['type'] = (string)$v['type'];   //弹幕样式  
            $arr[$k]['cid'] = (string)$v['cid'];  //现在是弹幕id，以后可能是发送者id了
            $arr[$k]['ip'] = (string)$v['ip'];  //弹幕ip
            //$arr[$k][] = (string)$v['time'];  //弹幕系统时间
            $arr[$k]['sertime'] = $date = date('m-d H:i',$v['time']);  //弹幕系统时间
            $arr[$k]['size'] = (string)$v['size'];  //弹幕系统大小
        }
